...
// --- Tunnel Config: loaded from /config when served by the Quest Android app ---
// When the Quest app's NanoHTTPD is serving, /config returns {"wsUrl": "https://xxx.trycloudflare.com"}
// When running on PC (bot.py), /config doesn't exist and we fall back to same-origin WebSocket.
let _tunnelWsHost = null;
let _tunnelConfigPromise = null;

async function _loadTunnelConfig() {
  try {
    const resp = await fetch('/config');
    const config = await resp.json();
    if (config.wsUrl) {
      // Strip protocol prefix to get just the host (e.g., "xxx.trycloudflare.com")
      _tunnelWsHost = config.wsUrl.replace(/^https?:\/\//, '').replace(/\/$/, '');
      console.log('[Aura] Quest app mode — WebSocket will connect to tunnel:', _tunnelWsHost);
    } else {
      console.log('[Aura] /config returned empty wsUrl — using same-origin WebSocket (PC mode)');
    }
  } catch (e) {
    console.log('[Aura] No /config endpoint — using same-origin WebSocket (PC mode)');
  }
}
_tunnelConfigPromise = _loadTunnelConfig();
...
async function startJourney(options = {}) {
  ...
  // Determine WebSocket target: tunnel (Quest app) or same-origin (PC)
  await _tunnelConfigPromise; // Ensure tunnel config is loaded before connecting
  let wsProtocol, wsHost;
  if (_tunnelWsHost) {
    // Quest app mode: connect to Cloudflare tunnel for voice AI
    wsProtocol = 'wss:';
    wsHost = _tunnelWsHost;
    console.log('[Aura] Connecting WebSocket to Cloudflare tunnel:', wsHost);
  } else {
    // PC/browser mode: connect to same host (existing behavior)
