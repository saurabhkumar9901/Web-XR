...
# Local Aura MCP server (mcp_server.py), run alongside this process.
...
async def call_mcp_tool(mcp_client: MCPClient | None, tool_name: str, arguments: dict) -> dict:
    """Calls a tool on the local Aura MCP server (mcp_server.py) and returns its
    structured result as a dict.

    The MCP server only computes *what should happen* (the client message to
    dispatch, what Aura should say next) — it never touches a websocket. This
    function is the boundary: callers apply the result to the live websocket
    and pipecat state themselves. If the MCP server is unreachable or the call
    fails, this logs the error and returns {} so a hiccup there never crashes
    the live voice session; callers fall back to sane defaults.
    """
    ...
    try:
        ...
        print(f"[Aura] MCP tool call '{tool_name}' failed: {e}")
        return {}
...
async def websocket_endpoint(websocket: WebSocket, voice: str = "Despina", resume: str = "false"):
    ...
    # Connect to the local Aura MCP server for the lifetime of this session.
    # AsyncExitStack lets us hold the connection open without wrapping the
    # rest of this (long) function body in an `async with` block.
    mcp_exit_stack = AsyncExitStack()
    try:
        mcp_client = await mcp_exit_stack.enter_async_context(MCPClient(MCP_SERVER_URL))
    except Exception as e:
        print(f"[Aura] Could not connect to the Aura MCP server at {MCP_SERVER_URL}: {e}")
        print("[Aura] Continuing without it — tool calls will fall back to their built-in defaults.")
        mcp_client = None
    ...
    async def trigger_xr_scene(params: FunctionCallParams):
        ...
        # Business logic (validation + message shape + LLM prompt) lives on
        # the MCP server; this handler just applies the result locally.
        mcp_result = await call_mcp_tool(mcp_client, "trigger_xr_scene", {
        ...
        # Set meditation flag via global or passed processor reference
        guidance_processor.is_meditating = mcp_result.get("set_meditating", True)

        # Return prompt to enforce the 5-phase guided meditation arc
        await params.result_callback({
            "value": mcp_result.get(
                "llm_prompt",
                (
                    f"The {sub_type} visual is now live. Guidance mode: "
                    f"{guidance_category}/{guidance_subcategory}. "
                    "BEGIN YOUR GUIDED MEDITATION NOW. Speak 2-3 slow, soothing sentences "
                    "for Phase 1 (Arrival) and Phase 2 (Body Settling), then stop speaking."
                ),
            )
        })
    ...
    async def show_environment_menu(params: FunctionCallParams):
        ...
        # MCP server ensures the top pick is included and dedupes to 3 entries.
        mcp_result = await call_mcp_tool(mcp_client, "show_environment_menu", {
        ...
        menu_state["guidance_category"] = msg.get("guidance_category", guidance_category)
        menu_state["guidance_subcategory"] = msg.get("guidance_subcategory", guidance_subcategory)
        menu_state["reason"] = msg.get("reason", reason)
        ...
        await params.result_callback({
            "value": mcp_result.get(
                "llm_prompt",
                "[SYSTEM: The environment menu is now visible on the user's screen. Do NOT speak or say anything right now. Stay completely silent. The user is choosing an environment. You will be notified when they make a choice.]",
            )
        })
    ...
    async def stop_xr_tour(params: FunctionCallParams):
        ...
        await params.result_callback({
            "value": mcp_result.get("llm_prompt", "Tour stopped. Aura is back in the center.")
        })
    ...
    async def show_voice_menu(params: FunctionCallParams):
        ...
        mcp_result = await call_mcp_tool(mcp_client, "show_voice_menu", {})
        msg = mcp_result.get("message", {"action": "show_voice_menu"})
        try:
            await websocket.send_text(json.dumps({"type": "app-message", "data": msg}))
        ...
        await params.result_callback({
            "value": mcp_result.get(
                "llm_prompt",
                "[SYSTEM: The voice menu is now visible on the user's screen. Do NOT speak or say anything right now. Stay completely silent while they choose.]",
            )
        })
